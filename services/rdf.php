...
if ($result->NumRows() == 1)
{
	...
	while (!$result->EOF) 
	{
		...
		{
			$citation->author = $result->fields['author'];
		}	
		if ($result->fields['year'] != '')
		{
			$citation->year = $result->fields['year'];
		}	
		
		if ($result->fields['article-title'] != '')
		{
			$citation->{'article-title'} = $result->fields['article-title'];
		}	
		if ($result->fields['journal-title'] != '')
		{
			$citation->{'journal-title'} = $result->fields['journal-title'];
		}	
		if ($result->fields['ISSN'] != '')
		{
			$citation->ISSN = $result->fields['ISSN'];
		}	
		if ($result->fields['volume'] != '')
		{
			$citation->volume = $result->fields['volume'];
		}	
		if ($result->fields['issue'] != '')
		{
			$citation->issue = $result->fields['issue'];
		}	
		
		if ($result->fields['first-page'] != '')
		{
			$citation->{'first-page'} = $result->fields['first-page'];
		}	
		if ($result->fields['last-page'] != '')
		{
			$citation->{'last-page'} = $result->fields['last-page'];
		}	
		...
		{
			$citation->unstructured = $result->fields['unstructured'];
		}	
			
		
		$reference->citation[$citation->key] = $citation;
